...
import { onRequestValidateEmail, validateEmail } from "@/services/auth";
import Token from "../ui/token/Token";

interface Props {
  form: UseFormReturnType<
    FormValuesType,
    (values: FormValuesType) => FormValuesType
  >;
  mutation: UseMutationResult<any, any, any, unknown>;
  handleSignup: () => void;
}

const SignupForm: React.FC<Props> = ({ form, mutation, handleSignup }) => {
  const [message, messageSet] = React.useState<string | boolean>(false);
      ...
      <Box className="mt-8 space-y-6">
        ...
        <Box>
          <Input
            ...
            type="email"
            onBlur={async () => {
              const response = await onRequestValidateEmail({
                email: form.values.email,
              });
              if (response) messageSet(response);
            }}
            ...
            label={
              <Text className="text-[#525252] font-[500]">
                ...
                <Link
                  target="_blank"
                  href="/#terms_of_use"
                  className="text-[#FAA61A] hover:underline"
                >
                  Terms & Conditions
                </Link>{" "}
                and{" "}
      ...
      <Token
        ...
        onSubmit={async (data) => {
          ...
          if (response) {
            ...
            toast.error("Please check your email to confirm token.");
          }
        }}
      />
    </Form>
